...
class GithubApiImplementation(GithubApi):
    ...
    def ensure_authenticated(self: Self) -> None:
        ...
        if code != 0:
            print(
                "Not authenticated. Running 'gh auth login' (follow prompts in browser)..."
            )
    ...
    def add_variable_to_environment(
        ...
        print(
            f"Adding variable '{name}' with value='{value}' to environment '{environment}' "
        ...
        if code != 0:
            if "409" in err:
                ...
                return
            print(
                f"Could not add variable '{name}' to environment '{environment}': {err}"
            )
