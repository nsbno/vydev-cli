...
def test_upgrade_aws_repo_cancelled(cli_handler, mock_deployment_migration, string_io, monkeypatch):
    ...
    output = string_io.getvalue()
    assert "Upgrade AWS Repo" in output
    assert "Upgrading AWS repo..." not in output


def test_upgrade_aws_repo_error(cli_handler, mock_deployment_migration, string_io, monkeypatch):
...
def test_upgrade_application_repo_success(cli_handler, mock_deployment_migration, string_io, monkeypatch):
    ...
    mock_deployment_migration.find_terraform_infrastructure_folder.return_value = terraform_folder
    ...
    monkeypatch.setattr('rich.prompt.Prompt.ask', lambda *args, **kwargs: 
                        "test-app" if "name" in args[0] else 
                        ApplicationBuildTool.PYTHON if "build tool" in args[0] else
                        ApplicationRuntimeTarget.LAMBDA if "runtime" in args[0] else
                        "latest" if "version" in args[0] else
    ...
    assert "Creating parameter store version parameter..." in output
    assert "Parameter store version parameter created successfully!" in output


def test_upgrade_application_repo_folder_not_found(cli_handler, mock_deployment_migration, string_io, monkeypatch):
    """Test application repo upgrade when folder is not found."""
    # Mock error in finding terraform folder
    mock_deployment_migration.find_terraform_infrastructure_folder.side_effect = FileNotFoundError("Folder not found")
    ...
    mock_deployment_migration.upgrade_terraform_application_resources.assert_called_once_with(terraform_folder)
...
def test_main_aws_operation(monkeypatch, string_io):
    ...
    monkeypatch.setattr('deployment_migration.handlers.cli.CLIHandler', lambda *args, **kwargs: mock_cli_handler)

    # Mock Console to use StringIO
    monkeypatch.setattr('deployment_migration.handlers.cli.Console', lambda *args, **kwargs: Console(file=string_io, highlight=False))
